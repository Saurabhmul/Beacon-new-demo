...
## Recent Changes
- 2026-03-05: Fixed AI returning "NOT SURE" for affordability/willingness — root causes: 1) `formatCustomerData()` now handles `_payments`/`_conversations` underscore-prefixed keys and skips all object values from generic output to prevent `[object Object]` garble, 2) brain-template.txt rewritten with step-by-step NMPC calculation instructions (failed payments=$0=VERY LOW, NOT SURE only when zero data), 3) output-schema.json mandates non-empty `reason_for_affordability` and `reason_for_willingness` with calculation evidence
